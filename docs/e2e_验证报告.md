...
## 六、增量：可答性门控（2026-06-08 新增，**后于**上面这次 10 例真跑）
> 诚实边界：本节能力在 **上面那次 2026-06-07 的 10 例真跑之后** 才加入，**未**包含在那次 trace 里；
> 下表/上文的历史结果不代表门控行为，**不得**据此声称"门控已在 10 例 e2e 中验证"。

- **是什么**：RAG 链路在 `rerank` 之后、生成之前新增一道**确定性可答性门控** `answerability_check`——
  重排 top1 相关性分 ≥ 阈值（默认 0.15）判"可答"走原链路，过低/空召回判"不可答"转
  `low_confidence_answer` 诚实兜底（不调 LLM）。详见 [ARCHITECTURE.md](ARCHITECTURE.md) §6.4 与
  [能力地图.md](能力地图.md) §2.1。
- **当前验证级别（如实）**：**离线**单元测试 + 流式回归（**不需** infra）。
  - `tests/test_answerability.py`：高分可答 / 低分不可答 / 空召回 / 重排降级兜底 / fp16 下溢消歧（floor）/
    总开关 / 阈值可调 / 条件边 / 兜底措辞，共 10 例；
  - `tests/test_streaming_regression.py`：新增"不可答分支"用例（判不可答→跳过 references→发兜底 answer_delta）。
  - 全量 `pytest` **104 passed**（裸机无 langgraph 时流式用例 importorskip 跳过，属预期）。
- **复跑可见**：`scripts/run_e2e_examples.py` 已把 `answerability` 事件纳入 trace（每例多打印一行
  "可答性: 可答=… 置信=… (reason)"）。若按"数据准备"重新灌库并真跑，即可在管线内观测门控逐例结论。
- **尚未做**：未在真实 infra 上专门构造"不可答"语料跑端到端验证（演示语料下稠密召回恒有近邻、
  重排分通常高于阈值，难自然触发"不可答"分支）——这是与"联网兜底未触发"同类的、设计内的诚实边界。
