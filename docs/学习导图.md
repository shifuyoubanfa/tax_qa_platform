...
flowchart TD
  ...
  R7 -- "SSE: answerability" --> R7b{"⑧ answerability_check<br/>重排top1分≥阈值? 确定性可答性门控"}
  R7b -- "可答" --> R8["⑨ build_context + summarize_stream<br/>意图化取材(社保/编码结构化优先) → 逐token生成"]
  R7b -- "不可答(分太低/空召回)" --> R8b["low_confidence_answer<br/>诚实兜底话术(不调LLM·零编造)"]
  R8 -- "SSE: references → answer_delta × N" --> SAVE["⑩ save_memory → MongoDB 写回本轮"]
  R8b -- "SSE: answer_delta(兜底)" --> SAVE
...
**SSE 事件时序**：`intent → retrieval(各阶段) → answerability(可答性门控结论) → references →（text2sql 分支为 sql）→ answer_delta×N → done`（任意阶段异常发 `error`；判不可答时跳过 references、由 low_confidence_answer 直接发兜底 answer_delta）。
